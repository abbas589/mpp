...
public class Problem {
    ...
    public static List<String> elementsInBoth(List<String> list1, List<String> list2) {
        //implement
        List<String> elementsInBoth = new ArrayList<>();
        list1.stream().forEach(v -> {
            if (list2.contains(v)) {
                elementsInBoth.add(v);
            }
        });
    ...
    public static List<String> getZipsOfSpecialCustomers(List<Customer> list) {
        ...
        return list.stream().sorted(Comparator.comparing(Customer::getZip))
                .filter(v -> v.getCity().length() >= 6)
                .filter(city -> !city.getCity().contains("e"))
                .map(Customer::getZip)
